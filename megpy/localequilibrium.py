...
class LocalEquilibrium():
    def __init__(self,param,equilibrium,x_loc,x_label='rho_tor',n_x=10,n_theta=7200,n_harmonics=1,incl_analytic_geo=False,opt_bpol=False,opt_deriv=False,diag_lsq=0,verbose=True):
        self._params = {'miller':{
                         ...
                         'fourier':{
                             'param':self.fourier,
                             'param_jr':self.miller_jr,
                             'param_initial':list(np.zeros(2))+([.5,0.,0.,.75]+[0.,0.,0.,0.]*(n_harmonics-1)),#+list(np.random.rand(4*n_harmonics)),
        ...
        for i_x_loc,xfs in enumerate(self.x_grid):
            with np.errstate(divide='ignore',invalid='ignore'):
                ...
                if self._param == 'mxh':
                    ...
                    print('params_initial {}: {}'.format(i_x_loc,self.param_initial))
                ...
                print('params {}: {}'.format(i_x_loc,self.params))
    ...
    def miller_bp(self,shape,shape_deriv,theta,R,dpsidr,method='simplified'):
        # define the parameters
        [R0,Z0,r,kappa,delta] = shape
        [dR0dr,dZ0dr,s_kappa,s_delta] = shape_deriv
        with np.errstate(invalid='ignore'):
    ...
    def cost_param(self,params):
        if self._param in ['mxh']:
            # update the flux-surface reference coordinates
            self.R_ref = self.fs['R'][:-1] - params[0]
            self.Z_ref = self.fs['Z'][:-1] - params[1]

            # update the theta-grid
            self.theta = arcsin2pi(self.Z_ref/(params[2]*params[3]))
